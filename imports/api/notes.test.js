...
if (Meteor.isServer) {
    describe('notes', function() {
        ...
        it('should not update note if invalid id', function() {
            const title = 'updated title';
            expect(() => {
                Meteor.server.method_handlers['notes.update'].apply({ userId: noteOne.userId }, [ { _id: 'unvalidId', title }])
            }).toThrow();            
        });
        ...
        it('should return zero notes', function() {
            const res =  Meteor.server.publish_handlers.notes.apply({ userId: 'not found in the db' });
            const notes = res.fetch();           
